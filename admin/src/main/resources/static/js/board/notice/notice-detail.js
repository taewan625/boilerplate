window.addEventListener('load', () => {
    ...
    $('#btn-update').click(()=>{
       window.location.href = `/board/notice/${boardId}/modify`;
    });
            ...
            if(res.success){
                Object.entries(Object.keys(res.data)).forEach(([key, value])=>{
                    switch (value){
                        ...
                        case 'attached':
                            if(res.data[value]){
                                $(`#${value}`).html(`<button class="btn btn-outline-primary border-0" id="btn-download">${res.data['filename']} <i class='bx bx-download' ></i></button>`);
                            }else{
...
const getBoardId = () => {
    ...
    const match = path.match(/\/board\/notice\/(\d+)/);
